...
class Pointer final : public LPointer {
	...
	void
	pointerSwipeBeginEvent(const LPointerSwipeBeginEvent &event) override;
	void
	pointerSwipeUpdateEvent(const LPointerSwipeUpdateEvent &event) override;
	void pointerSwipeEndEvent(const LPointerSwipeEndEvent &event) override;

	void
	pointerPinchBeginEvent(const LPointerPinchBeginEvent &event) override;
	void
	pointerPinchUpdateEvent(const LPointerPinchUpdateEvent &event) override;
	void pointerPinchEndEvent(const LPointerPinchEndEvent &event) override;

	void
	pointerHoldBeginEvent(const LPointerHoldBeginEvent &event) override;
